...
import com.yusufteker.planora.feature.home.data.paging.FeedRemoteMediator
...
class FeedPagingSource(
    ...
    private val query = localDatabase.planoraDatabaseQueries.getAllPosts(ownerId = ownerId, topic = topic, limit = 0, offset = 0)
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, PostEntity> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val count = localDatabase.planoraDatabaseQueries.countAllPosts(ownerId = ownerId, topic = topic).executeAsOne()
                val data = localDatabase.planoraDatabaseQueries.getAllPosts(ownerId = ownerId, topic = topic, limit = limit, offset = offset).executeAsList()
...
class FeedRepository(
    ...
    suspend fun toggleBookmark(postId: String): Result<Unit> {
        ...
        val currentPost = localDatabase.planoraDatabaseQueries.getAllPosts(ownerId = ownerId, topic = null, limit = 1, offset = 0)
