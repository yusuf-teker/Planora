package com.yusufteker.planora.feature.home.data.mapper

import com.yusufteker.planora.core.database.PlanoraDatabaseQueries
...
fun PlanoraDatabaseQueries.insertTaskFromDto(task: TaskDto, isSynced: Long) {
    insertTask(
        id = task.id,
...
fun PlanoraDatabaseQueries.insertTaskFromRequest(
    id: String,
    creatorId: Long,
    ...
    insertTask(
        ...
        participants = request.participants.takeIf { it.isNotEmpty() }?.let { map ->
            val list = map.map { com.yusufteker.planora.shared.api.TaskParticipantDto(it.key, it.value, "") }
