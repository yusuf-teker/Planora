...
import com.yusufteker.planora.server.database.tables.TaskParticipantsTable
                        ...
                        if (existingTask != null) {
                            if (existingTask.creator.id.value == userId) {
                                ...
                                val participantsList = UsersTable.selectAll().where { UsersTable.id inList request.participants.keys }.map {
                                    com.yusufteker.planora.shared.api.TaskParticipantDto(
                    ...
                    if (taskEntity != null) {
                        ...
                        val isRoomMember = if (sharedRoomIds.isNotEmpty()) {
                            PlanRoomMembersTable.selectAll().where { 
                                ...
                                (PlanRoomMembersTable.status eq com.yusufteker.planora.shared.api.RoomMemberStatus.ACCEPTED) 
                            }.count() > 0
                        } else false
            ...
            get {
                ...
                val tasks = dbQuery {
                    ...
                    val filteredEntities = entities.filter {
                        if (it.type == com.yusufteker.planora.shared.api.TaskType.FOLDER) return@filter true
                    ...
                    filteredEntities.map { entity ->
                        ...
                        val participantsList = (TaskParticipantsTable innerJoin com.yusufteker.planora.server.database.tables.UsersTable).selectAll()
                            .where { TaskParticipantsTable.taskId eq entity.id.value }
                            .map { 
                                com.yusufteker.planora.shared.api.TaskParticipantDto(
                                    userId = it[TaskParticipantsTable.userId],
                                    name = it[com.yusufteker.planora.server.database.tables.UsersTable.name],
                                    avatarId = it[com.yusufteker.planora.server.database.tables.UsersTable.avatarId],
                                    profileImageUrl = it[com.yusufteker.planora.server.database.tables.UsersTable.profileImageUrl]
                                ) 
                            }
