...
import com.yusufteker.planora.server.database.tables.CommentEntity
...
object DatabaseSeeder {
    fun seed() {
        transaction {
            ...
            if (com.yusufteker.planora.server.database.tables.FollowerEntity.count() == 0L) {
                com.yusufteker.planora.server.database.tables.FollowerEntity.new {
                    follower = dummyUser!!
            ...
            if (com.yusufteker.planora.server.database.tables.BookmarkEntity.count() == 0L) {
                val posts = PostEntity.all().limit(5).toList()
                posts.forEach { post ->
                    com.yusufteker.planora.server.database.tables.BookmarkEntity.new {
                        user = dummyUser!!
                        this.post = post
                        createdAt = Instant.now()
                    }
