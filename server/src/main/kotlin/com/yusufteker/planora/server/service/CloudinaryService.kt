...
object CloudinaryService {
    ...
    suspend fun uploadProfileImage(imageBytes: ByteArray, userId: Int): String {
        return withContext(Dispatchers.IO) {
            ...
            val publicId = "planora_user_${userId}_${UUID.randomUUID()}"
    ...
    suspend fun uploadRoomImage(imageBytes: ByteArray, roomId: String): String {
        return withContext(Dispatchers.IO) {
            val publicId = "planora_room_${roomId}_${UUID.randomUUID()}"
            val uploadResult = cloudinary.uploader().upload(
                imageBytes,
                ObjectUtils.asMap(
                    "public_id", publicId,
    ...
    suspend fun deleteImageByUrl(url: String) {
        withContext(Dispatchers.IO) {
            try {
                // Example URL: https://res.cloudinary.com/cloud_name/image/upload/v1234/profile_pictures/planora_user_1_uuid.jpg
