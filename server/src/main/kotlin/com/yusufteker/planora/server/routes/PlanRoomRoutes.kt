...
import com.yusufteker.planora.server.database.tables.PlanRoomEntity
import com.yusufteker.planora.server.database.tables.PlanRoomMembersTable
import com.yusufteker.planora.server.database.tables.PlanRoomsTable
...
import com.yusufteker.planora.shared.api.PlanRoomMemberDto
import com.yusufteker.planora.shared.api.RespondToInviteRequest
    ...
    authenticate("auth-jwt") {
        route("/rooms") {
            ...
            post("/{roomId}/invitations/respond") {
                ...
                var pushCreatorId: Int? = null
                var pushRoomName: String = ""
                ...
                val success = dbQuery {
                    ...
                    val respondingUser = UserEntity.findById(userId)
                    if (room != null && respondingUser != null) {
                        pushCreatorId = room.creator.id.value
                        pushRoomName = room.name
                ...
                if (success) {
                    if (pushCreatorId != null && pushCreatorId != userId) {
                        val isAccepted = request.accept
                        com.yusufteker.planora.server.service.FcmService.sendPushToUser(
                            userId = pushCreatorId,
                            title = if (isAccepted) "Oda Daveti Kabul Edildi" else "Oda Daveti Reddedildi",
